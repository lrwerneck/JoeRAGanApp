...
#The process for the Retrieval Agent is the same as before with a few minor adjustments.

#First connect to the hosted weviate stores and create the langchain vectorstores. (Before we inserted the embeddings, this time just connext to an already existing Weviate Store)

#Second Create the Retriever Tools

#Third Create the agent - This will need to be a session_state variable because every user needs a different agent.
# import weaviate
...
#    st.session_state.client = weaviate.Client(
#         url="https://streamlit-llm-hackathon-vb29y6cc.weaviate.network",
#         auth_client_secret=auth_config,
#         additional_headers={
#             "X-OpenAI-Api-Key": "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"
#         }
#     )
...
# if 'chunk_vectorstore' not in st.session_state:
#    st.session_state.chunk_vectorstore = Weaviate(st.session_state.client, index_name = "Chunk_Node", text_key = "chunk_body", attributes=['ep_title','ep_link'])

# if 'topic_vectorstore' not in st.session_state:
#    st.session_state.topic_vectorstore = Weaviate(st.session_state.client, index_name = "Topic_Node", text_key = "topic")

# llm = ChatOpenAI(temperature=0,openai_api_key=st.secrets.OPENAI_API_KEY, model="gpt-3.5-turbo-16k", streaming=True)
...
embeddings = OpenAIEmbeddings(openai_api_key="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")
